...
//线程池,可以提交变参函数或lamba表达式的匿名函数执行,可以获取执行返回值
...
class threadpool {
    ...
    thr_oncreate_cb _oncreate_cb;
    thr_ondestroy_cb _ondestroy_cb;

public:
    inline threadpool(unsigned short size = 4, thr_oncreate_cb oncreate_cb = nullptr, thr_ondestroy_cb ondestroy_cb = nullptr, bool detachable = false) {
...
public:
    ...
    void discardOldestTask(int n = 1) {
        lock_guard<mutex> lock{_lock};
        while (n-- > 0) {
            _tasks.pop();
        }
